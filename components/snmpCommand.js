        ...
        async function (x) {
            if (x.target.id !== undefined) {
                if (!props.ips.some((y) => y.name === x.target.id)) {
                    await props.setIps([
                        ...props.ips,
                        { key: props.ips.length + 1, name: x.target.id, value: x.target.value },
                    ]);
                } else {
                    const ipList = [...props.ips];
                    const itemIndex = ipList.findIndex((object) => {
                        return object.name === x.target.id;
                    });
