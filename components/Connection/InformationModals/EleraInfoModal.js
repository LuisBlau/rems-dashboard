...
export default function EleraInfoModal({ modalData, eleraModalOpen, handleEleraModalClose, handleEleraModalOpen }) {
    ...
    if (modalData.EleraServices && Object.values(modalData.EleraServices)[0].containers) {
        return (
            <Grid item xs={12}>
                ...
                <Modal open={eleraModalOpen} onClose={handleEleraModalClose} aria-labelledby="modal-modal-title">
                    <Box sx={style}>
                        <Typography marginBottom={3} fontWeight={'Bold'} fontSize={'h6.fontSize'}>
                            ...
                            {new Date(
                                JSON.parse(
                                    Object.values(modalData.EleraServices)[0]
                                ).containers[0].lastModifiedTimestamp
                            ).toDateString()}
                        </Typography>
                        <Container maxWidth="lg" className={classes.container}>
                            <Grid container spacing={3}>
                                {eleraServices.map((service, index) => (
