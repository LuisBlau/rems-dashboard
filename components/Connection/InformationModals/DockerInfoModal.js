            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                rows.push(
                    objectifyRow(
                        ...
                        arr[i].CPUUsage,
                        arr[i].Image,
                        Math.trunc(arr[i].MemoryUsage / bytesPerMegabyte) + 'mb'
                    )
                );
            }
        }
            ...
            return (
                <Grid item xs={12}>
                    <Tooltip arrow title="Docker Information">
                        <IconButton onClick={handleDockerModalOpen}>
                            <DirectionsBoat style={{ color: '#484848' }} cursor={'pointer'} />
                        </IconButton>
                    </Tooltip>
                    <Modal open={dockerModalOpen} onClose={handleDockerModalClose} aria-labelledby="modal-modal-title">
                        <Box sx={style}>
