...
export default function CreateDeploymentConfig() {
    ...
    const handleSubmit = (event, asCommon) => {
        ...
        let url = (asCommon === true && context.userRoles.includes('toshibaAdmin')) ? `/api/sendCommand?retailerId=common` : `/api/sendCommand?retailerId=${selectedRetailer}`;

        axios.post(url, commandObj)
