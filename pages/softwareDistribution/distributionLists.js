...
export default function DistributionLists() {
    ...
    function filterBySelectedVersion() {
        ...
        if (selectedVersion !== undefined && selectedVersion !== null) {
            if (selectedVersion.includes('agentSelectVersion: ')) {
                const parsedSoftware = getStringBetween(
                    selectedVersion,
                    'agentSelectSoftware: ',
                    ' agentSelectVersion:'
                );
                ...
                for (let i = 0; i < preFilteredData.length; i++) {
                    if (preFilteredData[i].agent.versions) {
                        for (let j = 0; j < _.size(preFilteredData[i].agent.versions); j++) {
                            ...
                            if (
                                Object.keys(preFilteredData[i].agent.versions[j])[0] === parsedSoftware &&
        ...
        if (fileExtension === 'csv') {
            reader.onload = async ({ target }) => {
                ...
                setSelectedAgents(
                    _.concat(
                        selectedAgents,
                        _.filter(importListAgents, (i) => !_.includes(selectedAgents, i))
                    )
                );
            };
        } else {
    ...
    const handleAllRight = () => {
        setSelectedAgents(
            _.concat(
                selectedAgents,
                _.filter(availableAgents, (i) => !_.includes(selectedAgents, i))
            )
        );
    };
        ...
        try {
            if (newStoreList) {
                if (listName !== '') {
                    ...
                    throw new Error('Distribution Lists must have a name')
                }
            } else if (_.includes(Object.keys(selectedExistingList), '_id')) {
                ...
                throw new Error('Must Create a New Distrubtion List or Select an Existing to Update')
            }
    ...
    function DistributionListGridItem() {
        if (storeOnlyView) {
            return (
                <Grid item xs={2.5}>
                    {customList('In Distribution List', _.uniqBy(selectedAgents, 'storeName'))}
                </Grid>
            );
        } else {
            return (
                <Grid item xs={2.5}>
                    {customList('In Distribution List', selectedAgents)}
                </Grid>
            );
        }
    }
    ...
    function CustomListItemDisplayName({ agent, labelId }) {
        ...
        return <ListItemText id={labelId} primary={displayName} />;
    }
